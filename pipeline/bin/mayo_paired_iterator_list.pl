...
my ($sName, $sInFile1, $sInFile2, $sOutFile);
my ($fpLST, $fpOUT);
my ($nI, $nN, $sPrefix);
...
if (defined $hCmdLineOption{'input_file_list'}) {
	...
	while (<$fpLST>) {
		...
		($sName, $sInFile1, $sInFile2) = split(/:|\t/, $_);
...
if ((defined $hCmdLineOption{'input_file1'}) && (!(defined $hCmdLineOption{'input_file2'}))) {
	...
	print $fpOUT "\$;I_FILE_BASE\$;\t\$;INPUT_FILE_1\$;\$;SAMPLE_NAME\$;\n";

	for ($nI = 0; $nI<@aInputFiles1; $nI++) {
		($_, $_, $sPrefix) = File::Spec->splitpath($aInputFiles1[$nI]);
		print $fpOUT "$sPrefix\t$aInputFiles1[$nI]\t$hCmdLineOption{'sample_name'}\n";
	}
}
...
if ((defined $hCmdLineOption{'input_file1'}) && (defined $hCmdLineOption{'input_file2'})) {
	...
	print $fpOUT "\$;I_FILE_BASE\$;\t\$;INPUT_FILE_1\$;\t\$;INPUT_FILE_2\$;\$;SAMPLE_NAME\$;\n";

	for ($nI = 0; $nI<@aInputFiles1; $nI++) {
		($_, $_, $sPrefix) = File::Spec->splitpath($aInputFiles1[$nI]);
		print $fpOUT "$sPrefix\t$aInputFiles1[$nI]\t$aInputFiles2[$nI]\t$hCmdLineOption{'sample_name'}\n";
	}
}
